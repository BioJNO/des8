...
# 
# # remove the lines we don't need (data from unrelated experiments)
# colnames(merged)
# 
# # a list of strings contained in the sample names we want
# want <- c("BW248",
#           "Plant",
#           "Barke")
# 
...
###############################################################################
# If running from FigShare data start from here -------------------------------
###############################################################################

prefilter <- read.csv("des8_50K_pre-filter.csv")
colnames(prefilter)

nomono_parents <- prefilter[prefilter[,4] != prefilter[,5],]
